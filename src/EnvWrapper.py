...
class SingleAgentTrainingWrapper(gym.Wrapper):
    """Abstracts away the opponent's turn using self.opponent.pick_action making UTTT seem singleplayer"""
    def __init__(self, env, opponent_pool_paths, swap_to_newest_model_prob: float=0.8):
        ...
        if self.opponent_pool_paths:
            ...
            self.opponent_path = self.opponent_pool_paths[-1]
        else:
            self.opponent = FrozenAgentOpponent(name="opponent_container")
            self.opponent_path = "default_opponent"
    ...
    def step(self, action):
        ...
        if self.unwrapped.game.apply_action(action): # If learning agent's move just ended the game
            agent_won = self.unwrapped.game._check_3x3_state(self.unwrapped.game.mini_board_states.reshape(3, 3)) == self.learning_agent
            rewards = 1.1 if agent_won else 0
            info = self.unwrapped._get_info()
            info["opponent_path"] = self.opponent_path

            return self.unwrapped._get_obs(), rewards, True, False, info
        ...
        return self.unwrapped._get_obs(), rewards, terminated, False, info
    ...
    def reward_func(self, old_board_states, terminated):
        if terminated:
            ...
            if winner == -self.learning_agent: return -1.1
            else: return 0 # Tie, agent winning is checked in step()
