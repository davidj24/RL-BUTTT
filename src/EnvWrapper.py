...
class SingleAgentTrainingWrapper(gym.Wrapper):
    ...
    def reset(self, seed: Optional[int]=None, options: Optional[dict]=None):
        ...
        if self.env.game.current_player != self.learning_agent:
            action_mask = obs[3, :, :].flatten()
            action = self.opponent.pick_action(obs, action_mask)
            self.env.game.apply_action(action)
        
        return self.env._get_obs(), self.env._get_info()

    def step(self, action):
        old_board_states = self.env.game.mini_board_states.copy()
        if self.env.game.apply_action(action): # If learning agent's move just won the game
            return self.env._get_obs(), 1, True, False, self.env._get_info()

        obs = self.env._get_obs()
        action_mask = obs[3, :, :].flatten()
        action = self.opponent.pick_action(obs, action_mask)
        terminated = self.env.game.apply_action(action)
    ...
    def reward_func(self, old_board_states, terminated):
        if terminated:
            ...
            macro_state = self.env.game.mini_board_states.reshape(3, 3)
            winner = self.env.game._check_3x3_state(macro_state)
        ...
        reward = 0
        if np.any((old_board_states != self.env.game.mini_board_states) & (self.env.game.mini_board_states == self.learning_agent)):
